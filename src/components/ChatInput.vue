<template>
    <div class="rounded-4xl w-[94%] h-auto min-h-[60px] border border-gray-300 shadow-sm 
        ...
        <textarea :disabled="!isLoggedIn" @keydown.enter.prevent="doSendMessage" v-model="message" ref="ta"
            placeholder="询问任何问题" rows="1" @input="autoResize"
            class="p-2 w-full resize-none leading-6 text-md font-medium focus:outline-none bg-transparent"></textarea>
...
<script setup>
import { ref, nextTick, computed, defineEmits, onUpdated } from "vue";
...
const { setSelectedChatId, setIsReloading } = chatStore
...
const emit = defineEmits(['send-success', 'new-chat-start'])
function autoResize() {
    if (!ta.value) return
    // 1) 先还原到自适应起点
...
const doSendMessage = async () => {
    ...
    if (!selectedChatId.value) {
        const res = await newChat({ "chat_name": message.value })
        if (res) {
            newChatId = res.chat_id
            // 不要立即设置selectedChatId，等消息发送成功后再设置
            setIsReloading(true)
        } else {
            return
        }
    ...
    const messageData = {
        chat_id: newChatId || selectedChatId.value,
        message_content: message.value,
        is_ai: 0,
    }
    const res = await sendMessage(messageData)
    if (res) {
        // 如果是新聊天，先通知即将设置新的聊天ID
        if (newChatId) {
            emit('new-chat-start')
