<template>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
        @click.self="$emit('closeRegisterModal')">
        <!-- Modal Card -->
        <div class="relative w-full max-w-sm md:max-w-md bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <form class="px-6 py-6 space-y-4">
                ...
                <button type="submit"
                    class="w-full py-2 rounded-2xl bg-gradient-to-r from-gray-600 to-black hover:opacity-90 text-white font-medium">
                    注册
                </button>
            </form>
            ...
            <div class="px-6 py-4 text-center text-sm">
                已有账号？
                <a href="#" class="text-indigo-600 hover:underline" @click.prevent="$emit('goToLogin')">
                    立即登录
                </a>
            </div>
        </div>
